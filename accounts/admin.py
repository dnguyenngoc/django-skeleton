...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        ('Important dates', {
            'fields': (
                'last_login', 'date_joined', 'created_at', 'updated_at', 'deleted_at'
            )
        }),
    )
    ...
    def soft_delete_users(self, request: HttpRequest, queryset: QuerySet[User]) -> None:
        ...
        self.message_user(
            request,
            f'{count} user(s) were successfully soft deleted.'
        )
    ...
    def restore_users(self, request: HttpRequest, queryset: QuerySet[User]) -> None:
        ...
        self.message_user(
            request,
            f'{count} user(s) were successfully restored.'
        )
    ...
    def get_actions(self, request: HttpRequest) -> dict[str, Any]:
        ...
        if ('deleted_at__isnull' in request.GET and 
            request.GET['deleted_at__isnull'] == 'False'):
